...
async function run() {
    ...
    let [schemaId, schema] = await indy.issuerCreateSchema(stewardDid, 'gvt', '1.2',
        ['age', 'sex', 'height', 'name']);
    ...
    let [faberTranscriptCredDefId, cred_def_json] = await indy.issuerCreateAndStoreCredentialDef(walletHandle, trustAnchorDid, schema, cred_def_tag, cred_def_type, cred_def_config);
    log('Credential definition: ', cred_def_json);
